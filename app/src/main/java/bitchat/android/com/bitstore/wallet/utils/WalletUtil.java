...
public class WalletUtil {
    ...
    public static Wallet restoreWallet(String seedCode,PeerAddress[] peerAddresses){

        NetworkParameters params = Constants.NETWORK_PARAMETERS;
       String path= BaseApp.getContext().getFilesDir().getPath();
       String filePrefix="CMC_WALLET";
...
//        File chainFile = new File("restore-from-seed.spvchain");
        File chainFile = new File(path, filePrefix + ".spvchain");
        if (chainFile.exists()) {
            chainFile.delete();
        }
        ...
        if (peerAddresses != null) {
            for (int var6 = 0; var6 < peerAddresses.length; ++var6) {
                PeerAddress addr = peerAddresses[var6];
        ...
        try {
            File vWalletFile = new File(path, filePrefix + ".wallet");
