...
package com.android.wallet.offline;
...
import com.android.wallet.utils.Bluetooth;
    ...
    public final static class HttpPaymentTask extends DirectPaymentTask {
        ...
        public void send(final Payment payment) {
            super.backgroundHandler.post(new Runnable() {
                @Override
                public void run() {
                    Logger.i("trying to send tx to {}", url);
                    ...
                    try {
                        ...
                        if (response.isSuccessful()) {
                            ...
                            Logger.i("received {} via http", ack ? "ack" : "nack");
                            ...
                            Logger.i("got http error {}: {}", responseCode, responseMessage);
                            onFail(R.string.error_http, responseCode, responseMessage);
                        }
                    } catch (final IOException x) {
                        Logger.i("problem sending", x);
    ...
    public final static class BluetoothPaymentTask extends DirectPaymentTask {
        ...
        public void send(final Payment payment) {
            super.backgroundHandler.post(new Runnable() {
                ...
                @Override
                public void run() {
                    Logger.i("trying to send tx via bluetooth {}", bluetoothMac);
                    ...
                    try (final BluetoothSocket socket = device
                        ...
                        Logger.i("connected to payment protocol {}", bluetoothMac);

                        payment.writeDelimitedTo(os);
                        os.flush();

                        Logger.i("tx sent via bluetooth");
                        ...
                        Logger.i("received {} via bluetooth", ack ? "ack" : "nack");
